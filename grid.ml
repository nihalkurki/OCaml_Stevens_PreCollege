...
let set_red x = (Graphics.rgb x 0 0)
let set_blue x = (Graphics.rgb 0 0 x)
...
let color_box_cpu (x,y) color  =
let bcf = vb_cpu.(index x y)
in
  Graphics.set_color color;
  Graphics.fill_rect (bcf.x+bcf.bw) (bcf.y+bcf.bw)
    (bcf.w-(2*bcf.bw)) (bcf.h-(2*bcf.bw))
